...
public class ImageAdapter extends RecyclerView.Adapter<ImageAdapter.MyHodler> {
    ...
    @Override
    public void onBindViewHolder(@NonNull final MyHodler myHodler, final int position) {
        if (mList.get(position) instanceof String) {
            ...
            Uri uri = Uri.parse("res://com.lll.weidu/" + id);
            myHodler.image.setImageURI(uri);
        }
...
//                        myHodler.itemView.getContext().startActivityForResult(intent,101);
                    } else {
