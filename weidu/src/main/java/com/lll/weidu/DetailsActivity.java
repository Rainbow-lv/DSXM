...
import com.lll.weidu.adapter.ViewPageDetailsBannerAdapter;
import com.lll.weidu.bean.CreateBean;
...
import com.lll.weidu.presenter.AddToCar;
import com.lll.weidu.presenter.CommentPresenter;
import com.lll.weidu.presenter.CreateOrederPresenter;
import com.lll.weidu.presenter.DetailsPresenter;
import com.lll.weidu.presenter.ShoppingPresenter;
import com.lll.weidu.untils.UIUtils;
import com.lll.weidu.view.MyScrollView;
...
public class DetailsActivity extends WDActivity implements View.OnClickListener{
  ...
  //  private CreateOrederPresenter createOrederPresenter;
...
//        DetailsBeanPresenter.reqeust(commodityId);
     ...
     //   commentPresenter.reqeust(commodityId, 1, 5);
        new ShoppingPresenter(new MyC()).reqeust(userId, sessionId);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_cart:
                ...
                new AddToCar(new MyCall()).reqeust(userId, sessionId, s);
                break;
            case R.id.btn_buy:
               //创建订单
                //创建对象
                CreateBean createBean = new CreateBean();
//                int commodityId = createBean.getCommodityId();
//                int amount = createBean.getAmount();
                createBean.setCommodityId(commodityId);
                createBean.setAmount(1);
                List<CreateBean> list1 = new ArrayList<>();
                list1.add(createBean);
                Gson gson1 = new Gson();
                String s1 = gson1.toJson(list1);
                //创建对象
                createOrederPresenter = new CreateOrederPresenter(new CreatCall());
                createOrederPresenter.reqeust(userId,sessionId,s1,price,599);
                break;
        }
    }
...
//            Toast.makeText(getBaseContext(), "" + data.getResult(), Toast.LENGTH_SHORT).show();
    ...
    private class CreatCall implements DataCall<Result>{
        @Override
        public void success(Result result) {
            if (result.getStatus().equals("0000")) {
                Toast.makeText(DetailsActivity.this, result.getMessage(), Toast.LENGTH_SHORT).show();
            }
//            Toast.makeText(DetailsActivity.this, result.getMessage(), Toast.LENGTH_SHORT).show();
        }
        ...
        @Override
        public void fail(ApiException e) {
            Toast.makeText(DetailsActivity.this, e.getCode(), Toast.LENGTH_SHORT).show();
        }
    }
}
